...
% BC GSAS margin requirements:
%   Left and right margins must be 1.25 inches.
%   Top and bottom margins must be 1 inch.
%   Page numbers must be 1 inch from paper edge.
\usepackage[
    letterpaper,
    left=1.25in,
    right=1.25in,
...
% setspace: provides \singlespacing, \onehalfspacing, \doublespacing.
% Used by renaissance_title.tex to locally set single-line spacing on
% the frontispiece. Body text is double-spaced per BC GSAS / ProQuest
% dissertation formatting requirements.
\usepackage{setspace}
\doublespacing

% BC GSAS allows single-spacing for footnotes, block quotes, long tables,
% bibliography, and captions. Apply these exceptions:
% Note: footnotes are single-spaced by default with setspace, no fix needed.
% Note: \captionsetup{font={singlespacing}} is applied later, after
...
\usepackage{caption}
% Single-spaced captions per BC GSAS (body is double-spaced)
...
% Pretzel knot drawing macros (defines \crossing, \tangle, \pretzel,
% \bandhighlight and the colors thesisblue, thesisred, bandfill).
% Used for Figure \ref{fig:hedden_watson_family} in the introduction.
\input{tikz_pretzel_macros}
...
% etoolbox is already loaded near the top for single-spacing exceptions.
\AtEndEnvironment{vb}{\null\hfill$\diamond$}%
