...
\label{app:case analysis track1}
...
\subsubsubsubsubsection{Abscond}
...
This is Candidate Family 6. Absocond6 shows $\fbp$ ending immediately at $r$.


\subsubsubsubsubsection{Fervent}
...
\subsubsubsubsubsubsection{Panorama}
...
\subsubsubsubsubsubsection{Pristine}
...
\subsubsubsubsubsection{Gingham}
...
\subsubsubsubsubsection{Dossier}
...
\subsubsubsubsubsubsubsection{Querulous}
...
\subsubsubsubsubsubsubsection{Sybaritic}
...
Here $\fbd$ must continue with $r^+$ and to the left of $\fbs$ and $\fbi$. Otherwise, both $\fbi$ and $\fb$ cross $d$, and then continue with $p^-$ (or $\fbb$ will trace after crossing $\dbar$). But then at least one of $\fbs$ or $\fbi$ will trace. So we must have
\[
\fbs = \dbar r^+ b^+ o^*...
\]
