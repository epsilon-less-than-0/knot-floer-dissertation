...
\label{thm:splitting_termination}
    Let $(\tau, \psi, f)$ be a pseudo-Anosov map on $D_n$ carried by a standard train track $\tau$. There exists no infinite sequence of tight splittings starting from $\tau$.
\end{theorem}

\begin{proof}
    Suppose, for the sake of contradiction, that there exists an infinite sequence of tight splittings
    \[
    \tau = \tau_0 \to \tau_1 \to \tau_2 \to \dots
    \]
    starting from $\tau$, and let $\mu_i$ denote the invariant transverse measure on $\tau_i$.

    By Theorem~\ref{thm:autofolder_main}, the set of isomorphism classes of standard train tracks in a fixed stratum on $D_n$ is finite. Since each $\tau_i$ lies in the same stratum as $\tau$ (tight splitting preserves singularity data), the pigeonhole principle yields indices $k < m$ with $\tau_k \cong \tau_m$ as standard tracks.

    Fix such a pair $(k, m)$, and let $P_i$ denote the elementary folding matrix associated to each splitting $\tau_{i-1} \to \tau_i$, so that $\mu_{i-1} = P_i \mu_i$ via the pullback identification of edges. Composing along the cycle, we obtain $\mu_k = P\mu_m$, where $P = P_{k+1} \cdots P_m$ is a product of elementary folding matrices. Each $P_i$ has the form $I + E_i$, where $E_i$ is a non-negative integer matrix with a single nonzero entry (the elementary fold). Hence $P$ has all the entries of $I$ together with non-negative integer contributions; under the isomorphism $\tau_m \cong \tau_k$ identifying edges, $P$ acts on the weight vector and produces $\mu_k$ from $\mu_m$. Because the cycle has $m > k$, at least one $E_i$ contributes nontrivially, so under this isomorphism the weight vector $\mu_m$ has at least one entry strictly smaller than the corresponding entry of $\mu_k$. (This is the content of Corollary~\ref{cor:weight_reduction} applied to the composite of splittings around the cycle.)

    Now iterate the cycle indefinitely: because $\tau_m \cong \tau_k$, the same sequence of splittings can be performed again starting from $\tau_m$, yielding tracks $\tau_{2m-k} \cong \tau_m \cong \tau_k$ with $\mu_{2m-k}$ further reduced on the same distinguished edge orbit. Tracking this orbit through successive iterations, its weight forms a strictly decreasing sequence of positive numbers bounded below by $0$, hence converging to $0$. But the invariant measure of a pseudo-Anosov map is everywhere strictly positive on the real edges of any carrying train track, contradicting this limit.
...
As guaranteed by the termination of tight splitting (Theorem \ref{thm:splitting_termination}), this reduction process is both finite and exhaustive. In Chapter \ref{chap:stratatraintrackanalysis}, we will execute this procedure, utilizing comprehensive splitting diagrams to systematically collapse the raw output of our \texttt{autofolder} algorithm.
