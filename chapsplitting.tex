...
\begin{proof}
    ...
    However, by Corollary \ref{cor:weight_reduction}, each tight split at a loop switch strictly reduces the weight of at least one branch orbit. Composing these reductions over the cycle from $\tau_k$ to $\tau_m$ yields an isomorphism $\tau_m \cong \tau_k$ under which the weight vector $\mu_m$ has at least one entry strictly smaller than the corresponding entry of $\mu_k$. Iterating this cycle, the affected weights would form a strictly decreasing sequence bounded below by zero, hence converging to zero. But the invariant measure of a pseudo-Anosov map is everywhere positive, contradicting the existence of the unstable foliation.

    Thus, the sequence of tight splittings must terminate.
\end{proof}
