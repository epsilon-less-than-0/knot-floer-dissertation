...
The proof strategy for Theorem \ref{thm:main_detection} relies entirely on this geometric bridge. We will systematically assume the hypothetical knot $K$ belongs to each of Thurston's three geometric categories: torus, satellite, and hyperbolic. We will then eliminate all distinct knot candidates in each category. We note immediately that the torus knot case is entirely trivial; the Alexander polynomials of torus knots are completely classified, and none can possibly coincide with the observed polynomial $\Delta_K(t) = (t^2 - t + 1)^2$. Thus, the remaining proof rests entirely on eliminating the satellite and hyperbolic cases.
...
\end{lemma}
\begin{proof}
    Farber, Reinoso, and Wang \sidecite{farber2024fixed} classified all pseudo-Anosov monodromies on the genus-two surface with one boundary component and no interior fixed points. They showed that the only such monodromy arising from a fibered knot in $S^3$ belongs to the cinquefoil $T(2,5)$. The cinquefoil has a completely different knot Floer homology from $T_{2,3} \# T_{2,3}$, which is immediately deduced from the fact that $\Delta_{T(2,5)}(t) = t^4 - t^3 + t^2 - t + 1$.
\end{proof}
...
We perform a \textbf{real blow-up} at $r$. Concretely, this is the same operation that appeared in the proof of Proposition \ref{prop:capping off} under the name ``puncturing and identifying with a disk interior'': we remove the point $r$ from $S^2$, identify the punctured sphere $S^2 \setminus \{r\}$ with the interior $\mathrm{int}(D^2)$ of the closed unit disk, and extend continuously to the closed disk $D^2$ by including the boundary $\partial D^2$ as a new circle, which one can think of as the unit tangent circle at $r$. The six branch points $\{q_1, \dots, q_6\}$ become marked points in $\mathrm{int}(D^2) \subset D^2$. The map $\phi_b|_{S^2 \setminus \{r\}}$ extends continuously to a pseudo-Anosov homeomorphism $\phi_\beta \colon D^2 \to D^2$ of the closed disk that fixes $\partial D^2$ setwise. We define $\beta \in \mathrm{Mod}(D_6)$ to be its mapping class.

\begin{remark}[Why a fixed point licenses the blow-up]\label{rem:fixed_point_licenses_blowup}
The hypothesis $\phi_b(r) = r$ is precisely what allows the real blow-up at $r$ to produce a self-homeomorphism of the resulting disk. Real blow-up at an arbitrary point $r$ of a surface $S$ replaces $r$ with the unit tangent circle $UT_r S$, yielding a compact surface $\tilde S$ with a new boundary component $\partial_r$. Given a homeomorphism $f \colon S \to S$, the natural extension to the blow-up sends a tangent direction at $r$ to the corresponding tangent direction at $f(r)$, induced by the derivative of $f$ at $r$. If $f(r) \neq r$, then this extension sends $\partial_r$ to $\partial_{f(r)}$, that is, the blow-up at $r$ is mapped to the blow-up at $f(r)$ --- which is a different surface, even though it is canonically isomorphic to the first. To stay within a single surface --- to get an element of $\mathrm{Mod}(\tilde S)$ rather than a map between two distinct blow-ups --- one needs $f(r) = r$. In our setting, since $z$ and $p$ are fixed by $\hat{\phi}_h$ and descend to $r$ under $\pi$, the spherical map $\phi_b$ fixes $r$, and the blow-up at $r$ produces a self-map $\phi_\beta \colon D^2 \to D^2$. Because $r$ is a regular (non-singular) point of the foliations of $\phi_b$ (it is the image of two non-singular interior fixed points of $\hat{\phi}_h$), the local pseudo-Anosov dynamics near $r$ is the standard hyperbolic stretch-and-shrink in the two foliation directions; the induced action on $\partial D^2 \cong UT_r S^2$ is an orientation-preserving circle homeomorphism, which is the boundary behavior of $\phi_\beta$.
\end{remark}

\begin{remark}[Puncturing versus real blow-up]\label{rem:puncture_vs_blowup}
We pause to record the relationship between two operations that play the same logical role at different points in this dissertation. \emph{Puncturing} a surface $\Sigma$ at an interior point $r$ removes that point, yielding the non-compact surface $\Sigma \setminus \{r\}$. \emph{Real blow-up} at $r$ replaces the point $r$ with the unit tangent circle at $r$, yielding a compact surface with one new boundary component. These two operations produce different compact-versus-non-compact surfaces, but they carry the same topological-dynamical information for pseudo-Anosov maps fixing $r$: the punctured surface is homeomorphic to the interior of the blow-up, and a pseudo-Anosov map fixing $r$ extends naturally to a homeomorphism of the blow-up that fixes the new boundary circle setwise. Baldwin-Hu-Sivek frame their analogous construction in terms of puncturing; we have chosen to frame ours in terms of real blow-up because it yields the compact disk $D^2$ with marked points and a true boundary $\partial D^2$ --- the natural setting for the braid mapping class groups and train-track machinery developed in Chapter \ref{chap:train tracks}.
\end{remark}

\begin{remark}\label{rem:fulltwist_ambiguity}
The mapping class $\beta \in \mathrm{Mod}(D_6)$ obtained from this blow-up is not quite canonical: it is well-defined only up to powers of the full twist $\Delta_6^2 \in \mathrm{Mod}(D_6)$. The pseudo-Anosov representative $\phi_b$ fixes the point $r$, hence determines a class $[\phi_b] \in \mathrm{Mod}(S^2_{0,6}, r)$ in the pointed mapping class group of the sphere with $r$ as a basepoint (homeomorphisms required to fix $r$ as a point, but not pointwise on any neighborhood). The disk braid $\beta$ is a preimage of $[\phi_b]$ under the natural surjection $\mathrm{Mod}(D_6) \twoheadrightarrow \mathrm{Mod}(S^2_{0,6}, r)$, obtained by collapsing $\partial D^2$ to a point (which becomes $r$). The kernel of this surjection is generated by the Dehn twist about a curve parallel to $\partial D^2$ --- which is precisely the full twist $\Delta_6^2$, central in $\mathrm{Mod}(D_6)$ --- giving the short exact sequence
\[
1 \to \mathbb{Z}\langle \Delta_6^2 \rangle \to \mathrm{Mod}(D_6) \to \mathrm{Mod}(S^2_{0,6}, r) \to 1.
\]
The preimage of $[\phi_b]$ is therefore a coset of $\mathbb{Z}\langle \Delta_6^2 \rangle$, and any two choices of representative for $\beta$ differ by an integer number of full twists --- equivalently, by the local-dynamical data recorded along the boundary $\partial D^2$ at the blown-up point $r$. This ambiguity is benign for our purposes: the train-track enumeration developed in Chapter \ref{chap:train tracks} naturally operates on the quotient $\mathrm{Mod}(D_6)/\langle \Delta_6^2 \rangle$, as a consequence of Corollary \ref{cor:induce_by_braid_fulltwist}. We return to this point in Chapter \ref{chap:train tracks} after the train-track framework is in place.
\end{remark}
...
\]
There are two such lifts (related by composition with the covering involution $\iota$, which on $\Sigma_2^2$ swaps the two boundary components); we choose $\phi_\Phi$ to be the lift that preserves the boundary components individually rather than swapping them. The lift is pseudo-Anosov, since being pseudo-Anosov is detected at the level of the invariant foliations, which lift through the branched cover.

We claim that on the interior of $\Sigma_2^2$, $\phi_\Phi$ \emph{agrees on the nose} with the restriction of $\hat{\phi}_h$ to $\Sigma_2^0 \setminus \{z, p\}$, after the canonical identification of the interior of $\Sigma_2^2$ with $\Sigma_2^0 \setminus \{z, p\}$. To see this: by the construction of $\phi_\beta$ as the descent of $\hat{\phi}_h$ via $\pi$, the restriction $\hat{\phi}_h|_{\Sigma_2^0 \setminus \{z, p\}}$ is one of the two lifts of $\phi_b|_{S^2 \setminus \{r\}}$ under $\pi'$; the other lift is $\iota \circ \hat{\phi}_h|_{\Sigma_2^0 \setminus \{z, p\}}$, which sends $z$ to $p$ and $p$ to $z$. The first lift preserves the local neighborhoods of $z$ and $p$ separately and so corresponds to the lift $\phi_\Phi$ that preserves $\partial_z$ and $\partial_p$ individually; the second lift swaps them. Hence $\phi_\Phi$ and $\hat{\phi}_h$ agree on the punctured genus-two surface, and in particular agree on the interior of $\Sigma_2^2$.

Since $\mathrm{Fix}(\hat{\phi}_h) = \{z, p\}$, which are precisely the points blown up to form $\partial_z$ and $\partial_p$, the restriction $\hat{\phi}_h|_{\Sigma_2^0 \setminus \{z,p\}}$ has no fixed points in the interior, and therefore neither does $\phi_\Phi$. Thus, $\phi_\Phi$ is a fixed-point-free pseudo-Anosov representative on $\Sigma_2^2$.
