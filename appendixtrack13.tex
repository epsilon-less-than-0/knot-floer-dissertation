...
\label{app:case analysis track 13}
...
or trace:
\[
\fbo =...s^- \dbar r^+ b^- r^- d p^- i^+ p^+ \dbar r^+ b^+ r^_ d o^\times
\]
...
Here $\fbo$ can continue with $p^*$ or $s^_-$. Suppose it were the former. Then this forces $\fbi$ to continue with $p^-$ and to the right of the incoming $\fbo$. Otherwise $\fbo$ would be forced to continue with $p^+$ and eventually trace. 
