...
In the previous chapter, we successfully eliminated the pseudo-Anosov strata $(2; 1^4, 2^2; \emptyset)$, $(4; 1^6; \emptyset)$, and $(2; 1^6; 4)$ by exploiting the global orientability of their lifted invariant foliations. However, the remaining possible strata for the disk monodromy—specifically those containing odd-pronged internal singularities, such as $(2; 1^6; 3^2)$ and $(3; 1^6; 3)$—do not admit orientable foliations. To eliminate these final candidates, we cannot rely on the Alexander polynomial. Instead, we must directly construct the valid train track maps within these strata and manually apply the Trace Lemma established in Chapter \ref{chap:train tracks}.

This topological case analysis requires a finite, exhaustive list of candidate standard train tracks to serve as the starting point. While the existence of an invariant train track is guaranteed by the Bestvina-Handel algorithm \sidecite{bestvina1995train}, we require a systematic enumeration of all possible tracks within our specific strata. 

To achieve this, we utilize the concept of the \textit{folding automaton}, introduced by Ko, Los, and Song \sidecite{SongEntropy}. A folding automaton is a directed graph where vertices represent isomorphism classes of train tracks and edges represent folding operations. For the purposes of this thesis, we utilize the automaton strictly as an \textbf{enumeration engine}. By generating the complete automaton for a given stratum, the set of vertices provides the exact, finite catalog of standard train tracks required for our subsequent analysis.

In this chapter, we describe the theory of folding automata on the $n$-punctured disk $D_n$. We then present our computational implementation of the ``genauto'' algorithm proposed by Ham and Song \sidecite{HamSong2007}, which we have developed in the software package \texttt{autofolder} to systematically output the candidate tracks for our remaining strata.

\section{Folding and The Folding Automaton}
\label{sec:automaton_definition}

Ko, Los, and Song \sidecite{SongEntropy} defined the folding automaton $\mathcal{A}_n$ for the braid group $B_n$. This structure encodes all possible standard train tracks (of a fixed complexity) and the transitions between them. First, we must define the procedure of \emph{folding} a train track.

\subsection{Elementary Folding}
\label{subsec:elementary_folding}

To rigorously define the folding operation, we adopt the terminology of Reinoso \cite{reinoso2024pseudo}, who adapts the notion of elementary folding from Ham and Song \sidecite{HamSong2007}.
...
\begin{equation}\label{equation:folding decomposition}
...
It is further shown that the folding decomposition of Equation \eqref{equation:folding decomposition} can be rewritten so that it implies that the braid $\beta$ decomposes as the composition of standardizing braids associated to the elementary folding operations $p_i$, along with a braid inducing the isomorphism $\psi$:

\begin{equation}\label{equation:folding decomposition, braid}
...
\begin{corollary}
    Let $\beta$ be a pseudo-Anosov $n$-braid. Up to conjugacy, a decomposition of $\beta$ in the form of Equation \eqref{equation:folding decomposition, braid} appears as a cycle in the folding automaton.
\end{corollary}
\begin{myproof}
...
\subsection{Proof of Algorithmic Exhaustion}
\label{sec:autofolder_proof}

In the Introduction, we stated Theorem \ref{thm:autofolder_main}, asserting that the algorithmic realization of the folding automaton provides a complete and finite classification of all standard train tracks for a given stratum. We now formalize this proof.

\begin{proof}[Proof of Theorem \ref{thm:autofolder_main}]
    The proof requires establishing two properties of the directed graph generation: \textit{exhaustiveness} (it finds all relevant tracks) and \textit{finiteness} (the algorithm terminates).
    
    \textbf{Exhaustiveness:} By the foundational work of Bestvina and Handel \sidecite{BestvinaHandel1992}, every invariant train track carrying a pseudo-Anosov homeomorphism can be constructed via a sequence of elementary folding operations starting from a standard invariant train track. Furthermore, any pseudo-Anosov map belonging to a fixed topological stratum (dictated by its singularity data) must be carried by a standard track possessing the exact corresponding polygon structure. 
    
    The algorithm is initialized with the finite set of base standard tracks matching the stratum's singularity profile. The algorithm branches by executing every geometrically valid elementary fold (left, right, and mixed) at every eligible switch on the track. Because the algorithm systematically explores every allowable edge transition in the folding complex, no train track capable of carrying a map in the given stratum can be omitted from the generated directed graph. Thus, the output is exhaustive up to train track equivalence.
    
    \textbf{Finiteness:} An elementary fold on a standard train track produces another train track. By collapsing degenerate edges and resolving trivial bigons, the resulting track is returned to standard form. Crucially, a standard train track on the $n$-punctured disk is a trivalent graph where the number of vertices $V$ and real edges $E$ are strictly bounded by the Euler characteristic of the punctured disk and the singularity data. 
    
    Because the number of vertices and edges is globally bounded for a fixed stratum, there are only finitely many graph isomorphism classes of standard train tracks on the surface. The algorithm checks each newly generated train track against a registry of previously visited isomorphism classes. If a fold yields an isomorphism class already in the registry, the algorithm creates a directed edge to the existing node rather than spawning a new graph. 
    
    Since the state space of isomorphism classes is finite, the branching process must eventually close on itself, forming strongly connected components. Therefore, the algorithm strictly avoids infinite generation and terminates, yielding a finite folding automaton.
\end{proof}


\subsection{Completeness and Connectivity}
\label{sec:completeness}

A natural question arises regarding the completeness of the \texttt{genauto} algorithm: does exploring the graph via directed folding edges guarantee that we find all (isomorphism classes of) standard train tracks of the stratum?

The affirmative answer relies on the connectivity of the complex of train tracks, as established by Penner and Harer \sidecite{penner1992combinatorics}. We formalize this completeness guarantee in the following theorem.

\begin{theorem}[Completeness of the \texttt{genauto} Algorithm]
    Let $\mathcal{S}$ be a connected stratum of the space of measured foliations on $D_n$. Given a seed standard train track $\tau_{seed} \in \mathcal{S}$, the \texttt{genauto} algorithm exhaustively enumerates every isomorphism class of standard train tracks in $\mathcal{S}$.
\end{theorem}

\begin{myproof}
    By the work of Penner and Harer \sidecite{penner1992combinatorics}, the space of measured laminations $\mathcal{ML}_0(D_n)$ admits a cellular decomposition where cells correspond to standard train tracks. Crucially, Penner and Harer \cite[Theorem~2.3.1]{penner1992combinatorics} prove that the set of all standard train tracks forms a connected complex. Specifically, they show that any two standard tracks in the same stratum are related by a finite sequence of \emph{splitting} moves and ambient isotopies.

    To translate these geometric moves into the combinatorial language of folding automata:
    \begin{itemize}
        \item \textbf{Splitting is Inverse Folding:} The operation of \emph{splitting} a train track (as defined in Penner--Harer) is the topological inverse of the elementary folding operation. If a track $\tau$ folds to $\tau'$, then $\tau'$ splits to $\tau$. Thus, a splitting move corresponds to traversing a directed edge of the automaton backwards.
        \item \textbf{Connectivity of the Graph:} Since any two tracks in the stratum are related by a sequence of splits and folds, the folding automaton is connected as an undirected graph.
    \end{itemize}

    The \texttt{genauto} algorithm implements a Breadth-First Search (BFS) on the directed graph of the automaton. Starting from $\tau_{seed}$, it computes the set of all tracks reachable by one fold (the neighborhood $N(\tau_{seed})$). Inductively, at step $k$, it finds all tracks reachable by paths of length $k$. Because the graph is finite (as proven previously) and connected, the reachable component of $\tau_{seed}$ is the entire graph $\mathcal{A}$. Thus, the algorithm necessarily terminates having enumerated every possible standard train track in the stratum.
\end{myproof}

\section{Computational Implementation: \texttt{autofolder}}
\label{sec:implementation}
The relevance of folding automata to our work on the detection of the connect-sum of trefoils lies in our analysis of the non-orientable strata $(2;1^6;3)$ and $(2;1^6;3^2)$ on $D_6$. Specifically, having an implementation of the \texttt{genauto} algorithm will allow us to generate all possible isomorphism classes of standard train tracks $[\tau]$ that may fit into geometric data $(\beta, \phi_\beta, \tau, f_\beta)$ belonging to those strata.
...
Crucial to the \texttt{genauto} algorithm is the detection of train track isomorphism. Specifically, when the algorithm generates a new track (Line 9), it must determine if this track corresponds to a state already existing in the automaton. To perform this check algorithmically, we rely on the discrete data of the graph structure.
...
\begin{myproof}
    ...
    \textbf{Step 1: Constructing the Surface Homeomorphism.}
    The tuple $(G, \mathcal{O})$ defines a unique ribbon graph structure. Following Penner and Harer \sidecite[Section 1.1]{penner1992combinatorics}, this combinatorial data uniquely determines an oriented surface with boundary $N(\tau)$ obtained by thickening the graph. Since $\phi$ preserves the cyclic ordering $\mathcal{O}$, it induces an orientation-preserving homeomorphism $h': N(\tau_1) \to N(\tau_2)$ between the regular neighborhoods of the tracks.

    \textbf{Step 2: Extending to the Disk.}
    The complement $D_n \setminus N(\tau)$ consists of a set of disjoint polygons. The isomorphism $\phi$ induces a boundary-preserving map between these complementary regions. We extend $h'$ to a homeomorphism $h: D_n \to D_n$ by mapping the interiors of the regions of $\tau_1$ to those of $\tau_2$ using the Alexander trick.
    
    Crucially, we must address the marked points. By the definition of a standard track, the boundary of each punctured region contains a unique ``vertical'' infinitesimal edge. Since $\phi$ preserves edge labels, it maps the vertical edge of $\tau_1$ to the corresponding vertical edge of $\tau_2$. We extend $h$ radially from the boundary to the marked point $q_i$, ensuring that $h(q_i)$ is mapped to the corresponding marked point in $\tau_2$.

    \textbf{Step 3: Defining the Braid.}
    The map $h$ constructed above is an orientation-preserving homeomorphism of $D_n$. It fixes the boundary $\partial D_n$ pointwise (due to the rigidity of the outer infinitesimal edges) and permutes the set of marked points $Q$. Therefore, $h$ defines an element of the mapping class group $\mathrm{Mod}(D_n) \cong B_n$. Let $\beta = [h]$ be the braid class represented by $h$.
    
    By construction, $h(\tau_1) = \tau_2$ as embedded graphs. Since the action of the braid $\beta$ is defined by applying a diffeomorphism isotopic to $h$, it follows that $\beta(\tau_1)$ is isotopic to $\tau_2$.
\end{myproof}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% COMMENTED OUT FOR THESIS: EXTENDED AUTOMATON CAPABILITIES (CODE)  %%%
%%% (Standardization, Transition Matrices, Full Graph Algorithm)      %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\iffalse
...
In this chapter, we formalized the theory of folding automata and provided a complete algorithmic realization of the enumeration framework originally theorized by Ko, Los, and Song \sidecite{SongEntropy}. As noted by Los \sidecite{LosInfiniteSequence}, a systematic study of all pseudo-Anosov homeomorphisms within a given stratum requires the rigorous development of its specific train-track automata theory. Furthermore, Los anticipated that because such automata are practically immense, an explicit algorithmic description is strictly necessary to make the state space tractable. 

For the remaining non-orientable strata on the disk—specifically $(2; 1^6; 3^2)$ and $(3; 1^6; 3)$—our \texttt{autofolder} implementation fulfills this exact requirement. It provides the exhaustive combinatorial foundation required to systematically hunt for valid train track maps, ensuring no candidate track is missed.
...
To bridge the gap between algorithmic enumeration and manual topological analysis, we must systematically filter this generated state space. In the next chapter, we introduce the geometric operation of \textit{tight splitting}. By applying tight splitting to our enumerated tracks, we can resolve these localized complexities and strictly reduce the automaton's massive output to a minimal, canonical set of jointless standard train tracks. It is upon this highly refined, manageable set of candidate tracks that our final Trace Lemma arguments will be executed.