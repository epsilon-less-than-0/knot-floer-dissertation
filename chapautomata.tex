...
By the work of Penner and Harer \sidecite{penner1992combinatorics}, the space of measured laminations $\mathcal{ML}_0(D_n)$ admits a cellular decomposition whose cells correspond to standard train tracks. The key connectivity result is \cite[Theorem~2.3.1]{penner1992combinatorics}: any two equivalent positively measured standard tracks $(\tau_1, \mu_1)$ and $(\tau_2, \mu_2)$ admit a common refinement $(\tau, \mu)$, in the sense that each $(\tau_i, \mu_i)$ splits (and isotopes) to $(\tau, \mu)$. Consequently, any two standard tracks in the same stratum are related by a finite sequence of splits and inverse splits (modulo isotopy).

To translate this geometric statement into the combinatorial language of folding automata:
\begin{itemize}
    \item \textbf{Splitting is inverse folding:} The operation of \emph{splitting} a train track (as defined in Penner-Harer) is the topological inverse of the elementary folding operation. If a track $\tau$ folds to $\tau'$, then $\tau'$ splits to $\tau$. A splitting move thus corresponds to traversing a directed edge of the automaton backwards.
...
\textbf{Finiteness.}
A standard train track on the $n$-punctured disk is a finite trivalent graph $G$ whose vertex and edge counts are determined by the stratum's singularity data via the Euler-Poincar\'{e} formula. The total number of switches and real edges admits a uniform bound depending only on $n$ and the singularity profile, and the cyclic ordering at each switch admits only finitely many configurations. Therefore the set of isomorphism classes of standard tracks in a fixed stratum is finite.
