...
\begin{proof}[Proof sketch of Lemma \ref{lem:trace_lemma}]
The fixed points of $\phi_\Phi$ are contained in the preimage of the fixed points of $\phi_\beta$. For an efficient train-track map, fixed points of the map on the graph correspond to fixed points of the homeomorphism.
...
The two lifts $\tilde{f}_h$ and $\iota \circ \tilde{f}_h$ act differently on this fiber: one fixes each point individually, while the other swaps them. By the canonical-lift condition (equivalently, by the same-side characterization or the $\partial_p$-preservation), the canonical lift fixes each point of the fiber individually. Consequently, both $y$ and $\iota(y)$ are fixed points of $\tilde{f}_h$, and they correspond to fixed points of the geometric representative $\phi_\Phi$ in the interior of $\Sigma_2^2$.

Thus, if the trace is non-zero, $\phi_\Phi$ retains an interior fixed point, contradicting the FPF hypothesis. The vanishing of the trace --- equivalently, the no-self-appearance condition for real edges --- is therefore necessary for $\phi_\Phi$ to be FPF. The full ``even number of side-swapping edges between adjacent occurrences'' refinement is Reinoso's strengthening of this no-self-appearance condition, which we will use in the case analysis of Chapter \ref{chap:stratatraintrackanalysis}.
\end{proof}
