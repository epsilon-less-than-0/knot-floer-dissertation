...
\begin{theorem}[{\cite{bestvina1995train}, \cite[Theorem~2.2.1]{reinoso2024pseudo}}] \label{theorem:bestvin-handel mapping class determined by train track map}
...
Applying Theorem \ref{theorem:bestvin-handel mapping class determined by train track map} to the specific case of maps on the $n$-marked disk, we have the following corollary:
...
We now restrict our attention to the unit disk $D_n$ with $n$ marked points $Q = \{q_1, \dots, q_n\}$. We utilize the subclass of \emph{standard train tracks} defined by Ko-Los-Song \sidecite{SongEntropy}, and Farber-Reinoso-Wang \sidecite{farber2024fixed} \sidecite{reinoso2024pseudo}. A result of Ko-Los-Song ensures that every pseudo-Anosov map on $D_n$ is carried by such a track.
...
\subsection*{Joints and Jointless Train Tracks on the Disk}
Most of the train tracks that will be studied in our work will have marked points enclosed by infinitesimal monogons. The work of Reinoso, Farber, and Wang \sidecite{farber2024fixed} shows that up to conjugacy, all homeomorphisms on $D_n$ are carried by train tracks where these monogons have no ``joints''.

\begin{definition}[Joint]
    Let $\tau$ be a standard train track. A \textit{joint} is a switch $v$ located on an infinitesimal monogon (a region surrounding a single marked point) such that $v$ is incident to more than one real edge.
\end{definition}

As shown in \cite{reinoso2024pseudo}, most pseudo-Anosov braids (mapping classes) are conjugate to maps carried by \textit{jointless} tracks, which simplifies the classification of strata in the space of measured foliations.
...
\begin{enumerate}
    \item \textbf{Real Edges:} Each real edge of $\tau$ avoids the branch points and therefore lifts to two distinct real edges in $\tilde{\tau}$, which are interchanged by $\tau_{hyp}$.
    \item \textbf{Interior Singularities:} An infinitesimal polygon with $k$ sides in the interior of $D^2$ (away from the marked points) lies in the unbranched region of the cover. It therefore lifts to a pair of disjoint $k$-sided polygons in $\tilde{\tau}$, swapped by $\tau_{hyp}$.
    \item \textbf{Boundary Behavior:} The boundary of the disk $\partial D^2$ lifts to the two boundary components of $\Sigma_2^2$, denoted $\partial_z$ and $\partial_p$, which are swapped by $\tau_{hyp}$. An infinitesimal $k$-gon adjacent to $\partial D^2$ lifts to a $2k$-gon bounding $\partial_z$ and a $2k$-gon bounding $\partial_p$, reflecting the local doubling of the cone angle.
\end{enumerate}

\begin{figure}[ht]
    \centering
    % \includegraphics[width=0.8\linewidth]{figures/reinoso_fig_2_5_lifting.png}
    \caption[Visual lift of a train track under the Birman-Hilden correspondence]{Visual representation of lifting a train track from the disk to the branched double cover. Image courtesy of Braeden Reinoso \cite{reinoso2024pseudo}.}
...
\]
satisfying the covering relation $\pi \circ \tilde{f} = \pi \circ \tilde{g} = f_\beta \circ \pi$. These two lifts are related by the covering involution: $\tilde{g} = \tau_{hyp} \circ \tilde{f}$.
...
Let $\beta \in \mathrm{Mod}(D_n)$ be a pseudo-Anosov braid carried by a standard train track $\tau$, and let $f_\beta: \tau \to \tau$ be the induced train track map. Let $\tilde{f}_h$ be the canonical symmetric lift of $\beta$ to the surface $\Sigma_2^2$ (the lift preserving the boundary components). If the pseudo-Anosov representative $\phi_\Phi$ is fixed-point free (FPF) in the interior of $\Sigma_2^2$, then the trace of the transition matrix $M(f_\beta)$ must be zero.
\end{lemma}
...
If $\mathrm{tr}(M(f_\beta)) > 0$, then $f_\beta$ has a fixed point on the interior of some real edge $e \in \tau$. This edge lifts to a pair of edges $e_1, e_2$ in $\tilde{\tau}$ which are swapped by $\tau_{hyp}$. A fixed point $x \in e$ lifts to a pair $\{y, \tau_{hyp}(y)\}$ in $\tilde{\tau}$. The canonical lift $\tilde{f}_h$ must map the fiber $\{y, \tau_{hyp}(y)\}$ to itself. Since $\tilde{f}_h$ commutes with $\tau_{hyp}$ but is distinct from $\tau_{hyp} \circ \tilde{f}_h$, careful analysis of the index and Nielsen classes shows that a fixed point on the quotient track forces fixed points on the surface for the canonical lift. Specifically, if the trace is non-zero, the geometric representative $\phi_\Phi$ retains a fixed point in the interior. Thus, the vanishing of the trace is a necessary condition for the map to be FPF.
\end{proof}
